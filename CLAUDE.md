...
- The newer windows (`AccessibleDraftWindow`, `AccessibleTournamentWindow`) skip the global dispatcher entirely and bind on `getRootPane().getInputMap(WHEN_IN_FOCUSED_WINDOW)`, which is scoped to the window by construction. Prefer that for anything new; `SideboardingHandler` and `AccessibleDeckEditorWindow` are the models.
