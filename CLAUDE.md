...
- **A refresh that changes nothing must not touch the list.** Every zone is rebuilt by a polling timer, and clearing and refilling a `JList` fires accessibility events at the screen reader whether or not anything differs. `ZoneListPanel.updateItems` drops such a refresh by comparing text, detail, source object and action type; `harness/src/xmageaccess/ui/ZoneListHarness.java` pins it. Keep new zone content comparable — put what the user hears in the display name rather than in a field the comparison cannot see.
